...
const GESTURE_BAR_Y = 8;
const GESTURE_BAR_HEIGHT = 20;
const VELOCITY_TICK_Y = 32;
const VELOCITY_TICK_HEIGHT = 20;
const VELOCITY_SPIKE_THRESHOLD = 15;
...
export function GestureTimeline({ entries, workspaceWidth }: GestureTimelineProps) {
  ...
  useEffect(() => {
    ...
    ctx.clearRect(0, 0, workspaceWidth, CANVAS_HEIGHT);
    ...
    const toX = (t: number) => ((t - windowStart) / WINDOW_MS) * workspaceWidth;
    ...
    entries.forEach((entry, i) => {
      ...
      ctx.fillRect(x, GESTURE_BAR_Y, barWidth, GESTURE_BAR_HEIGHT);
    });

    // Velocity spikes
    entries.forEach((entry) => {
      if (entry.velocity <= VELOCITY_SPIKE_THRESHOLD) return;

      const x = toX(entry.timestamp);
      if (x < 0 || x > workspaceWidth) return;

      ctx.fillStyle = 'rgba(255,189,46,0.9)';
      ctx.fillRect(x - 1, VELOCITY_TICK_Y, 2, VELOCITY_TICK_HEIGHT);
    });
    ...
    ctx.lineTo(workspaceWidth - 1, CANVAS_HEIGHT);
    ctx.stroke();
