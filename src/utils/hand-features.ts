...
import { sub3, normalize3, dot3, cross3, magnitude3, distance3d, clamp, centroid3 } from './geometry';
import { LANDMARK, FINGER_CHAINS, FINGERTIP_INDICES, PALM_INDICES, FEATURES } from '../config';
...
export function computeJointAngle(parent: Landmark, joint: Landmark, child: Landmark): number {
  const raw1 = sub3(parent, joint);
  const raw2 = sub3(child, joint);
  const mag1 = magnitude3(raw1);
  const mag2 = magnitude3(raw2);
  // Guard: zero-length bones (coincident points) → treat as straight (0)
  if (mag1 < 1e-6 || mag2 < 1e-6) return 0;
  const v1 = { x: raw1.x / mag1, y: raw1.y / mag1, z: raw1.z / mag1 };
  const v2 = { x: raw2.x / mag2, y: raw2.y / mag2, z: raw2.z / mag2 };
...
export function extractJointAngles(landmarks: Landmark[]): JointAngles {
  ...
  function fingerAngles(chainIdx: number): { mcp: number; pip: number; dip: number } {
    ...
    if (chainIdx === 0) {
      ...
      // CMC angle uses wrist as parent; MCP and IP use chain neighbors
      return {
        mcp: computeJointAngle(wrist, landmarks[chain[0]], landmarks[chain[1]]),
        pip: computeJointAngle(landmarks[chain[0]], landmarks[chain[1]], landmarks[chain[2]]),
        dip: computeJointAngle(landmarks[chain[1]], landmarks[chain[2]], landmarks[chain[3]]),
      };
    }
    // Non-thumb: chain = [MCP, PIP, DIP, TIP]
    return {
      mcp: computeJointAngle(wrist, landmarks[chain[0]], landmarks[chain[1]]),
      pip: computeJointAngle(landmarks[chain[0]], landmarks[chain[1]], landmarks[chain[2]]),
      dip: computeJointAngle(landmarks[chain[1]], landmarks[chain[2]], landmarks[chain[3]]),
...
export function extractPalmOrientation(landmarks: Landmark[]): PalmOrientation {
  ...
  const rawNormal = cross3(v1, v2);
  const rawMag = magnitude3(rawNormal);
  if (rawMag < 1e-6) return { pitch: 0, yaw: 0, roll: 0, normal: { x: 0, y: 0, z: 0 } };
  const normal = { x: rawNormal.x / rawMag, y: rawNormal.y / rawMag, z: rawNormal.z / rawMag };

  const pitch = Math.asin(clamp(-normal.y, -1, 1));
  const yaw = Math.atan2(normal.x, normal.z);

  // Roll: angle of palm lateral axis projected onto palm plane, measured against world-up
  const palmDir = normalize3(sub3(indexMcp, pinkyMcp));
  const proj = dot3(palmDir, normal);
  const inPlane = normalize3({
    x: palmDir.x - proj * normal.x,
    y: palmDir.y - proj * normal.y,
    z: palmDir.z - proj * normal.z,
  });
  const worldUp = { x: 0, y: -1, z: 0 };
  const upProj = dot3(worldUp, normal);
  const upInPlane = {
    x: worldUp.x - upProj * normal.x,
    y: worldUp.y - upProj * normal.y,
    z: worldUp.z - upProj * normal.z,
  };
  const upMag = magnitude3(upInPlane);
  let roll: number;
  if (upMag < 1e-6) {
    roll = 0;
  } else {
    const upNorm = { x: upInPlane.x / upMag, y: upInPlane.y / upMag, z: upInPlane.z / upMag };
    roll = Math.atan2(
      dot3(cross3(upNorm, inPlane), normal),
      dot3(upNorm, inPlane),
    );
  }
...
export function extractInterFingerSpread(landmarks: Landmark[]): [number, number, number, number] {
  const wrist = landmarks[LANDMARK.WRIST];
  const mcpIndices = [LANDMARK.THUMB_CMC, LANDMARK.INDEX_MCP, LANDMARK.MIDDLE_MCP, LANDMARK.RING_MCP, LANDMARK.PINKY_MCP];
...
export function extractThumbOpposition(
  ...
  return tips.map(idx => clamp(distance3d(thumbTip, landmarks[idx]) / safeSize, 0, 2)) as [number, number, number, number];
}
...
export function applyJointConstraints(angles: JointAngles): JointAngles {
  function clampFinger(joint: { mcp: number; pip: number; dip: number }) {
    return {
  ...
  function clampThumb(joint: { mcp: number; pip: number; dip: number }) {
    return {
      mcp: clamp(joint.mcp, 0, FEATURES.THUMB_MCP_FLEXION_MAX),
      pip: clamp(joint.pip, 0, FEATURES.THUMB_IP_FLEXION_MAX),
      dip: clamp(joint.dip, 0, FEATURES.THUMB_IP_FLEXION_MAX),
    };
  }

  return {
    thumb:  clampThumb(angles.thumb),
    index:  clampFinger(angles.index),
    middle: clampFinger(angles.middle),
    ring:   clampFinger(angles.ring),
    pinky:  clampFinger(angles.pinky),
  };
}
...
export function computeHandOpenness(landmarks: Landmark[], handSize: number): number {
  const palmCenter = centroid3(PALM_INDICES.map(i => landmarks[i]));
  const safeSize = Math.max(handSize, 0.001);
  let total = 0;
